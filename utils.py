...
class ConciseTweet:
    ...
    def __init__(self, line):
        ...
        if tweet and 'doc' in tweet:
            doc = tweet['doc']
            if 'coordinates' in doc and doc['coordinates'] and 'coordinates' in doc['coordinates']:
                self.coord = doc['coordinates']['coordinates']
            elif 'geo' in doc and doc['geo'] and 'coordinates' in doc['geo']:
                self.coord = doc['geo']['coordinates'][::-1]
            self.lang = doc['lang'] if 'lang' in doc and isinstance(doc['lang'], str) else None
